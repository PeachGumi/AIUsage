...
- Claude / Cursor / Kimi Code: 各公式ローカルクライアントの既存認証情報（読み取り専用）
- Antigravity: 実行中のローカルプロセスから検出したlocalhost endpointとCSRF token（保存しない）
- GitHub Copilot: `GH_TOKEN` / `GITHUB_TOKEN` またはGitHub CLIの既存token（保存しない）
- Z.AI: AIUsageがmacOS Keychainへ保存するCoding Plan API key
...
認証状態まで削除したい場合は、AIUsageが認証を所有するProviderの **Sign out** を使用してください。Codex / Claude / Antigravity / GitHub Copilot / Cursor / Kimiの外部認証は変更しないため、必要なら各公式クライアント側でログアウトします。
...
- Z.AI API keyはmacOS Keychainへ保存し、Sign out時に削除します。
- Claude / Cursor / Kimi / GitHub CLIの認証情報は読み取りのみで、AIUsageによるコピーを保存しません。
...
アプリ内のSign outで、AIUsageのWebKitデータストアにある対象サービスのWebデータまたはZ.AI API keyを削除できます。OpenCodeの保存済みworkspace IDもSign out時に削除します。外部クライアントの認証情報や旧アプリのファイル自体は変更・削除しません。
